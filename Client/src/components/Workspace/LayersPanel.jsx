...
const LayersPanel = () => {
  const { canvas, updateSelectedObject } = useWorkspace();
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0">
      <div className="flex-1 overflow-y-auto custom-scrollbar p-2 space-y-1 select-none px-1" data-selection={selectionVersion}>
        {userLayers.length === 0 ? (
          <div className="text-center text-[10px] font-bold text-app-muted uppercase tracking-widest mt-10">Слоев пока нет</div>
        ) : (
          userLayers.map((obj, i) => {
            ...
            let bgClasses = 'bg-app-elevated dark:bg-white/[0.02]';
            
            if (isSelected) {
              borderClasses = 'border-indigo-500';
              bgClasses = 'bg-indigo-50 dark:bg-indigo-600/20';
            }
            
            if (isDragOver && !isDragged) {
              borderClasses = 'border-indigo-400 border-dashed border-2';
              bgClasses = 'bg-indigo-100/60 dark:bg-indigo-500/10 scale-[1.02]';
            }
              ...
              <div 
                ...
                className={`group flex items-center justify-between p-3 rounded-xl cursor-pointer transition-all border ${borderClasses} ${bgClasses} ${
                  isDragged
                    ...
                    : 'shadow-sm text-app-text-secondary dark:text-slate-400 hover:bg-app-hover dark:hover:bg-white/5 hover:text-app-text dark:hover:text-slate-200'
                } ${isSelected ? 'text-indigo-700 dark:text-indigo-100' : ''}`}
              >
                <div className="flex items-center gap-3 overflow-hidden">
                  <div className="cursor-grab text-app-muted dark:text-slate-600 group-hover:text-app-text-secondary dark:group-hover:text-slate-400 active:cursor-grabbing px-1 -ml-2 shrink-0">
                  ...
                  <button onClick={(e) => toggleVisibility(obj, e)} className={`shrink-0 hover:text-app-text dark:hover:text-white transition-colors ${!obj.visible ? 'opacity-30' : ''}`}>
                  ...
                  <div className="shrink-0 w-3 h-3 rounded-sm border border-app-border-strong dark:border-white/20 flex items-center justify-center" style={{ backgroundColor: obj.type === 'image' ? 'transparent' : color }}>
                  ...
                  {isEditing ? (
                    <input
                      ...
                      className="text-xs font-bold bg-app-input dark:bg-black/40 text-app-text dark:text-white outline-none border border-indigo-500 rounded px-1 w-24 truncate focus:w-32 transition-all"
                    />
                  ) : (
                    <span className="text-xs font-bold truncate select-none text-inherit" title="Дважды кликните, чтобы переименовать">
                      {name}
                    </span>
                  )}
                </div>
                ...
                {!isEditing && (
                  <button 
                    ...
                    className="shrink-0 opacity-0 group-hover:opacity-100 p-1.5 text-app-muted hover:text-rose-500 dark:text-slate-500 dark:hover:text-rose-400 hover:bg-rose-500/10 rounded-lg transition-all"
                    title="Удалить слой"
                  >
